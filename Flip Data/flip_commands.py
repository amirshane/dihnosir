...
# Write result to a test file
f = open('strong_flip_dist.txt', 'w')
...
f = open('strong_flip_maxdist.txt', 'w')
...
f = open('weak_flip_dist.txt', 'w')
for l_ in weak_dist_labels:
    f.write(str(l_))
    f.write(' ')
f.close()

weak_maxdist_clustering = dihnosir.DIHNOSIR(n = 250, minPts_min = 3, silhouette_threshold = 0.6).fit(Y, unmerge = "weak")
weak_maxdist_labels = weak_maxdist_clustering.labels_
f = open('weak_flip_maxdist.txt', 'w')
