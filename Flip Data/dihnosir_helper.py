...
def cluster_constraint(labels):
    ...
    out_indices = indices.difference(in_indices)
    out_list = list(out_indices)
    
    separation_matrix = np.delete(separation_matrix, out_list, 0)
    separation_matrix = np.delete(separation_matrix, out_list, 1)
...
def similarity_matrix(subclusters):
    ...
    indices = [i for i in range(n_subclusters)]
    sim_matrix = np.asarray([0.0 for i in range(n_subclusters**2)])
    sim_matrix = np.reshape(sim_matrix, (n_subclusters, n_subclusters))
    pairs = itertools.combinations(indices, r = 2)
    for pair in pairs:
        x, y = pair[0], pair[1]
        x_set, y_set = set(subclusters[x]), set(subclusters[y])
        similarity = ssi(x_set, y_set)
        sim_matrix[x][y] += similarity
        sim_matrix[y][x] += similarity
...
def clusters(X, unmerged_subclusters, subcluster_connected_components):
    ...
    clusters_ = sorted(clusters_, key = len)[::-1]
